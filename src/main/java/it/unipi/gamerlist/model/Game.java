...
public class Game {
    ...
    public void insert(){
        ...
        if(ret == null){
            logger.log(Level.INFO, "ERROR insert game" + name + " in MongoDB");
            throw new RuntimeException("ERROR: it was't possible to insert the game in MongoDB");
        }

        //insert info in the graphDB if needed
        int res = addGameToGraph(this);
        if(res==-1)
            logger.log(Level.INFO, "ERROR insert game" + name + " in Neo4j");
    }
    ...
    public void update(){
        ...
        if(ret == null){
            logger.log(Level.INFO, "ERROR update game" + name + " in MongoDB");
        ...
        int res = addGameToGraph(this);
        if(res==-1)
            logger.log(Level.INFO, "ERROR update game" + name + " in Neo4j");
    }
    ...
    public void delete(){
        ...
        try {
            ...
            logger.log(Level.INFO, "ERROR delete game" + name + " from MongoDB");
        ...
        try {
            ...
            logger.log(Level.INFO, "ERROR delete reviews of " + name + " from MongoDB");
            System.err.println("ERROR: Unable to delete the review associated with the game due to an error: " + me);
        }

        //delete instance of the game from graphDB
        try ( Session session = Neo4jDriver.getInstance().getDriver().session() )
        {
            int res = session.writeTransaction((TransactionWork<Integer>) tx -> {
                Result r = tx.run( "MATCH (m:Game { name: $gamename}) DETACH DELETE m RETURN id(m)",
            ...
            if(res==-1)
                logger.log(Level.INFO, "ERROR delete game" + name + " from Neo4j");
        }
    ...
    /*
    Top k games with positive reviews for all genre in the last year
